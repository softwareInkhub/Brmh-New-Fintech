    ...
    <nav className="h-16 flex items-center px-4 md:px-6 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border-b border-gray-200/50 dark:border-gray-700/50 shadow-sm transition-all duration-300">
      ...
      <div className="flex items-center space-x-2 md:space-x-4">
        ...
        <div ref={profileRef} className="relative flex items-center gap-2">
          ...
          {/* Profile Dropdown */}
          {showProfileMenu && (
            <div className="absolute right-0 mt-2 w-64 bg-white/95 dark:bg-gray-800/95 backdrop-blur-sm border border-gray-200/50 dark:border-gray-700/50 rounded-xl shadow-lg z-50 animate-scale-in">
              <div className="p-3">
                {/* User Info */}
                <div className="px-3 py-3 text-sm text-gray-500 dark:text-gray-400 border-b border-gray-100 dark:border-gray-700 mb-2">
                  <div className="font-medium text-gray-900 dark:text-gray-100 truncate" title={user?.email}>{user?.email}</div>
                  {user?.username && (
                    <div className="text-xs text-gray-600 dark:text-gray-400 mt-0.5">@{user.username}</div>
                  )}
                  {user?.role && (
                ...
                <div className="space-y-1">
                  <button 
                    ...
                    className="w-full text-left px-3 py-2 hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 font-medium rounded-lg transition-colors duration-200 text-sm"
                  >
                    🔍 Debug Auth
                  </button>
