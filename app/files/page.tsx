...
function SlicePreviewComponent({ sliceData, file }: { sliceData: string[][]; file: FileData; selectedFields: string[] }) {
  ...
  const handleCheckDuplicates = async () => {
    ...
    try {
      ...
      console.log('Selected fields (original):', selectedFields);
      ...
      console.log('- Account ID:', file.accountId);
      console.log('- User ID:', userId);
      console.log('- Bank Name:', file.bankName);
      console.log('- API URL:', `/api/transactions?accountId=${file.accountId}&userId=${userId}&bankName=${encodeURIComponent(file.bankName || '')}`);
      ...
      console.log('API Response data type:', typeof existing);
      console.log('API Response is array:', Array.isArray(existing));
      console.log('API Response length:', existing?.length);
      console.log('API Response sample:', existing?.slice(0, 2));
      console.log('All existing transaction IDs:', existing?.map((tx: Record<string, unknown>) => tx.id));
      console.log('All existing transaction dates:', existing?.map((tx: Record<string, unknown>) => tx.Date));
      console.log('All existing transaction accounts:', existing?.map((tx: Record<string, unknown>) => ({ accountId: tx.accountId, accountName: tx.accountName, bankName: tx.bankName })));
      ...
      // Auto-deselect all duplicate rows
      ...
      // Stop here - enhanced detection disabled to respect user field selection
      console.log('Using original duplicate detection results only:', {
        totalDuplicates: allDupRows.size,
        fieldsUsed: uniqueFields
      });
      
      // Exit function here to prevent enhanced detection from running
      return;



      // Skip enhanced detection - use only the fields selected by user
      if (false) { // Disabled enhanced detection
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const batchSize = 200; // Process 200 transactions per batch
      ...
      for (let i = 0; i < rows.length; i += batchSize) {
        ...
        // Use regular endpoint for duplicate checking, fast endpoint when skipping
        const endpoint = payload.skipDuplicateCheck ? '/api/transaction/slice/fast' : '/api/transaction/slice';
        ...
        console.log(`Batch ${i}-${i + batchRows.length}: Status ${res.status}, Success: ${result.success}`);
        if (result.error) {
          console.log(`Batch error: ${result.error}`);
        }
        ...
        // Add 0.5 second sleep between batches (except for the last batch)
  ...
  useEffect(() => {
    const fetchAllUserFiles = async () => {
      ...
      try {
        ...
        const accountPromises = processedBanks.map(async (bank: Record<string, unknown>) => {
          try {
