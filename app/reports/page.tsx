...
export default function ReportsPage() {
  ...
  const [tagsSummary, setTagsSummary] = useState<{
    ...
    tags: Array<{
      ...
      statementIds: string[];
      transactions?: TransactionData[]; // Store individual transactions for this tag
    }>;
  } | null>(null);

  // Date filtering state
  const [reportDateFilter, setReportDateFilter] = useState<{
    type: 'all' | 'month' | 'year' | 'dateRange';
    month?: number;
    year?: number;
    startDate?: string;
    endDate?: string;
  ...
  const applyDateFilter = useCallback(async () => {
    if (!tagsSummary || reportDateFilter.type === 'all') {
    ...
    try {
      ...
      const params = new URLSearchParams({
        userId,
        filterType: reportDateFilter.type,
      });

      console.log(`🔍 reportDateFilter values:`, reportDateFilter);
      console.log(`🔍 reportDateFilter.month type:`, typeof reportDateFilter.month, 'value:', reportDateFilter.month);
      console.log(`🔍 reportDateFilter.year type:`, typeof reportDateFilter.year, 'value:', reportDateFilter.year);
      
      if (reportDateFilter.type === 'month' && reportDateFilter.month !== undefined && reportDateFilter.year !== undefined) {
        console.log(`📅 Adding month params: month=${reportDateFilter.month}, year=${reportDateFilter.year}`);
        params.append('month', reportDateFilter.month.toString());
        params.append('year', reportDateFilter.year.toString());
      } else if (reportDateFilter.type === 'year' && reportDateFilter.year !== undefined) {
        console.log(`📅 Adding year param: year=${reportDateFilter.year}`);
        params.append('year', reportDateFilter.year.toString());
      } else if (reportDateFilter.type === 'dateRange' && reportDateFilter.startDate && reportDateFilter.endDate) {
        console.log(`📅 Adding date range params: start=${reportDateFilter.startDate}, end=${reportDateFilter.endDate}`);
        params.append('startDate', reportDateFilter.startDate);
        params.append('endDate', reportDateFilter.endDate);
      } else {
        console.log(`❌ Missing required params for filter type: ${reportDateFilter.type}`);
        console.log(`❌ month: ${reportDateFilter.month} (${typeof reportDateFilter.month}), year: ${reportDateFilter.year} (${typeof reportDateFilter.year})`);
        console.log(`❌ startDate: ${reportDateFilter.startDate}, endDate: ${reportDateFilter.endDate}`);
        
        // Don't make the API call if required params are missing
        console.log(`❌ Skipping API call due to missing parameters`);
        setFilteredTagsSummary(null);
        return;
      }

      const url = `/api/reports/tags-summary-filtered?${params.toString()}`;
      console.log(`🚀 Calling filtered tags API: ${url}`);
      ...
      if (!response.ok) {
        ...
        console.error(`❌ Request params were:`, Object.fromEntries(params));
        
        // Don't throw error, just log it and return empty result
        console.log(`❌ Setting filtered tags summary to null due to API error`);
        setFilteredTagsSummary(null);
        return;
      }
      
      const filteredData = await response.json();
      console.log(`✅ Filtered tags summary loaded:`, filteredData);
      setFilteredTagsSummary(filteredData);
    } catch (error) {
  ...
  const resetDateFilter = useCallback(() => {
    setReportDateFilter({ type: 'all' });
    setFilteredTagsSummary(null);
  }, []);

  // Manual filter application - removed automatic filter on change
  ...
  const [transactionDateFilter, setDateFilter] = useState<string>('');
  ...
  const extractAmountAndTypeFromTx = (
    ...
    const filteredTagTransactions = useMemo(() => {
      ...
      if (transactionDateFilter) {
        ...
        if (!txDate || String(txDate) !== transactionDateFilter) {
          return false;
        }
      }
    ...
    return filtered;
  }, [activeTagTransactions, transactionDateFilter, drCrFilter, accountFilter, dateSortOrder]);
  ...
  const openTagTransactions = useCallback(async (tagName: string, openInNewTab: boolean = false) => {
    try {
      console.log(`🔍 Opening transactions for tag: ${tagName}, openInNewTab: ${openInNewTab}`);
      console.log(`🔍 Current reportDateFilter:`, reportDateFilter);
      
      const userId = typeof window !== 'undefined' ? localStorage.getItem('userId') : null;
      
      if (openInNewTab) {
        // Open in new tab with filtered data
        const baseUrl = `${window.location.origin}/transactions`;
        const params = new URLSearchParams({
          tag: tagName,
          userId: userId || '',
        });
        
        // Add date filter parameters if active
        if (reportDateFilter.type !== 'all') {
          params.append('filterType', reportDateFilter.type);
      ...
      console.log(`🎯 Setting modal state: tagName=${tagName}, loading=true`);
      ...
      console.log(`🔍 Final txs array has ${txs.length} transactions:`, txs);
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex gap-3">
            ...
            >
              ...
              {reportDateFilter.type === 'all' ? 'All Time' : 
               reportDateFilter.type === 'month' ? `${reportDateFilter.month}/${reportDateFilter.year}` :
               reportDateFilter.type === 'year' ? reportDateFilter.year :
               'Custom Range'}
            ...
            {reportDateFilter.type !== 'all' && (
              <button
                ...
                className="px-3 py-2 rounded-lg font-medium transition-all duration-200 flex items-center gap-2 bg-red-500 hover:bg-red-600 text-white"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className={`mb-6 p-4 rounded-lg border ${
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <select
                  value={reportDateFilter.type}
                  onChange={(e) => setReportDateFilter(prev => ({ ...prev, type: e.target.value as 'all' | 'month' | 'year' | 'dateRange' }))}
              ...
              {reportDateFilter.type === 'month' && (
                <>
                  <div>
                    ...
                    <select
                      value={reportDateFilter.month || ''}
                      onChange={(e) => setReportDateFilter(prev => ({ ...prev, month: e.target.value ? parseInt(e.target.value) : undefined }))}
                      className={`w-full px-3 py-2 rounded-lg border ${
                        theme === 'dark'
                          ? 'bg-gray-700 border-gray-600 text-gray-200'
                          : 'bg-white border-gray-300 text-gray-700'
                  ...
                  <div>
                    ...
                    <select
                      value={reportDateFilter.year || ''}
                      onChange={(e) => setReportDateFilter(prev => ({ ...prev, year: e.target.value ? parseInt(e.target.value) : undefined }))}
                      className={`w-full px-3 py-2 rounded-lg border ${
                        theme === 'dark'
                          ? 'bg-gray-700 border-gray-600 text-gray-200'
                          : 'bg-white border-gray-300 text-gray-700'
              ...
              {reportDateFilter.type === 'dateRange' && (
                <>
                  <div>
                    <label className={`block text-sm font-medium mb-2 ${
                      theme === 'dark' ? 'text-gray-300' : 'text-gray-700'
                    ...
                    <input
                      ...
                      onChange={(e) => setReportDateFilter(prev => ({ ...prev, startDate: e.target.value }))}
                      className={`w-full px-3 py-2 rounded-lg border ${
                        theme === 'dark'
                          ? 'bg-gray-700 border-gray-600 text-gray-200'
                          : 'bg-white border-gray-300 text-gray-700'
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setReportDateFilter(prev => ({ ...prev, endDate: e.target.value }))}
                      className={`w-full px-3 py-2 rounded-lg border ${
                        theme === 'dark'
                          ? 'bg-gray-700 border-gray-600 text-gray-200'
                          : 'bg-white border-gray-300 text-gray-700'
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[9999]">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-7xl mx-4 max-h-[95vh] flex flex-col relative">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-3">
                <button
                  onClick={() => activeTagName && openTagTransactions(activeTagName, true)}
            ...
            {(transactionDateFilter || drCrFilter || accountFilter || dateSortOrder) && (
